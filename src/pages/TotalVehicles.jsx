...
export default function TotalVehiclesPage() {
  ...
  const filtered = vehicles.filter((v) => {
    if (dateFilter && !String(v.entryTime || v.createdAt || '').startsWith(dateFilter)) return false;
          ...
          <Box sx={{ display: 'flex', gap: 1.5 }}>
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, background: isDark ? DARK.bg2 : 'rgba(255,255,255,0.7)', p: 0.5, px: 2, borderRadius: '16px', border: `1px solid ${isDark ? DARK.border : 'rgba(0, 0, 0, 0.2)'}` }}>
                <Typography variant="body2" fontWeight={700} sx={{ color: isDark ? DARK.text2 : '#4b5563' }}>Filter Date:</Typography>
                <DatePicker
